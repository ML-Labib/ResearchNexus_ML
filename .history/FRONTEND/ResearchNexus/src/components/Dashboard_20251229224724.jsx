...
const handleDeleteFolder = async (id) => {
  ...
  if (window.confirm('Move this folder to Recycle Bin?')) {
    try {
      // 2. CALL THE NEW BACKEND ENDPOINT (Soft Delete)
      // Note: Ensure port 9222 matches your server
      await axios.put(`http://localhost:9222/api/folders/trash/${id}`);
      ...
      alert("Error moving folder to Recycle Bin");
    }
  }
};
  ...
  // };
    ...
    const handleDeleteFile = async (id) => {
    // 1. Change the confirmation message
    if (window.confirm('Move this file to Recycle Bin?')) {
      try {
        // 2. Use PUT to move to trash (Soft Delete)
        // Ensure the port (9222) matches your server
        await axios.put(`http://localhost:9222/api/files/trash/${id}`);

        // 3. Refresh the UI
        if (selectedFolder) {
            loadFiles(selectedFolder.id); // Refresh file list
        }
        loadFolders(); // Refresh folder list (to update file counts)
        
      } catch (error) {
        console.error('Error moving file to trash:', error);
        alert("Failed to move file to Recycle Bin");
      }
    }
  };
